/**
 * co2-lookup.js — CO2-schatting o.b.v. euronorm + vermogensklasse
 *
 * Geëxporteerde functies (plain globals, geen ES modules vanwege content-script beperking):
 *   window.CIC_Lookups.estimateCO2(specs)  → { co2, method, confidence }
 *   window.CIC_Lookups.checkCO2Deviation(scraped, estimated, label)
 *
...
(function (root) {
  // -------------------------------------------------------------------------
  // Euronorm × brandstof × vermogensklasse → mediaan CO2 (g/km WLTP)
  // Brackets: [<75kW, 75-99kW, 100-129kW, 130-159kW, 160-199kW, 200+kW]
  //
  // Validatie (selectie):
  //   VW Golf 1.5 TSI 96kW  E6d petrol:  werkelijk 138, schatting 130  (−8)
  //   BMW 320d      140kW   E6d diesel:  werkelijk 118, schatting 122  (+4)
  //   BMW 530d      210kW   E6d diesel:  werkelijk 153, schatting 158  (+5)
  //   Audi RS6      441kW   E6d petrol:  werkelijk 249, schatting 230  (−19)
  //   BMW 320d      130kW   E5  diesel:  werkelijk 119, schatting 125  (+6)
  // -------------------------------------------------------------------------
  const EURO_POWER_TABLE = {
    ...
    diesel: {
      // Diesel CO2 daalt bij hogere vermogens door hoge thermische efficiëntie
  ...
  /**
   * @param {{ fuelType: string, euroNorm?: string, powerKw?: number, year?: number }} specs
   * @returns {{ co2: number, method: string, confidence: 'medium'|'low'|'very-low' }}
   */
  function estimateCO2(specs) {
    const { fuelType, euroNorm, powerKw, year } = specs;
    if (fuelType === "electric")
      return { co2: 0, method: "elektrische auto", confidence: "exact" };
  ...
  // Exporteer als namespace op window
  root.CIC_Lookups = { estimateCO2, checkCO2Deviation };
})(window);
